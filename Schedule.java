...
public class Schedule implements Serializable {
    ...
    public Schedule(TrainStation departLocation, TrainStation arriveLocation, LocalTime departTime,
            LocalTime arriveTime, Train trainOperated, double ticketPrice) {
        scheduleId = departLocation.getLocationName().substring(0, 3) + "-"
                + arriveLocation.getLocationName().substring(0, 3)
                + String.valueOf((int) (100000 + (Math.random() * (200000 - 100000 + 1))));
    ...
    public Schedule(TrainStation departLocation, TrainStation arriveLocation, LocalTime departTime,
            LocalTime arriveTime, double ticketPrice) {
        scheduleId = departLocation.getLocationName().substring(0, 3) + "-"
                + arriveLocation.getLocationName().substring(0, 3)
                + String.valueOf((int) (100000 + (Math.random() * (200000 - 100000 + 1))));
        this.departLocation = departLocation;
    ...
    public void addSchedule(Scanner scanner) throws Exception {
        ...
        do {
            ...
            if (userInput2 > departureIndex) {
                userInput2++; // Increment the input station number to account for the excluded departure
                              // station
            }
                    ...
                    if (departTime != null) {
                        ...
                        for (int j = 0; j < scheduleList.size(); j++) {
                            if (scheduleList.get(j).getDepartLocation().getLocationName()
                                    .equals(departLocation.getLocationName())) {
            ...
            do {
                ...
                if (BackendStaff.isValidTimeFormat(userInput)) {
                    ...
                    if (arriveTime != null) {
                        ...
                        for (int j = 0; j < scheduleList.size(); j++) {
                            if (scheduleList.get(j).getDepartLocation().getLocationName()
                                    .equals(arriveLocation.getLocationName())) {
            ...
            do {
                ...
                userInput2 = BackendStaff.validateIntegerInput(scanner,
                        "Enter the train number stated above (Press # to exit) > ");
                if (userInput2 == -1)
                    return;
            ...
            if (userInput.equalsIgnoreCase("Y")) {
                Schedule s = new Schedule(departLocation, arriveLocation, departTime, arriveTime, trainOperated,
                        ticketPrice);
                scheduleList.add(s);
    ...
    System.out.print("Do you confirm? (Y-Yes/ N-No) > ");confirm=BackendStaff.validateYNInput(scanner,"Do you confirm? (Y-Yes/ N-No) > ");
    ...
    public boolean isTimeConflict(Schedule schedule1, Schedule schedule2) {
        // Check if schedule1's departure time is between schedule2's departure and
        // arrival times
        if (schedule1.getDepartTime().compareTo(schedule2.getDepartTime()) >= 0 &&
                schedule1.getDepartTime().compareTo(schedule2.getArriveTime()) <= 0) {
            System.out.println("\nTRAIN INVOLVES IN OTHER SCHEDULE DURING THE TIME FRAME SELECTED.\n ");
        ...
        // Check if schedule1's arrival time is between schedule2's departure and
        // arrival times
